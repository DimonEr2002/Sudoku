from sudokufunc import valid, solve, find_empty
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit()  # выход
        board = [[0 for i in range(9)] for j in range(9)]  # пустая доска
            ...
            for j in range(9):
                if random.randint(1, 10) >= 5:
                    ...
                    if valid(board, (i, j), board[i][j]):
                        ...
                        board[i][j] = 0
        partialBoard = deepcopy(board)  # полная копия
        if solve(board):
            return partialBoard
...
class Board:
    ...
    # отменить предыдущий выбор поля
    ...
    def redraw(self, keys, wrong, time):
        ...
        if len(keys) != 0:
            for value in keys:
                self.tiles[value[0]][value[1]].display(keys[value], (21 + value[0] * 60, 16 + value[1] * 60),
                                                       (128, 128, 128))
        # вывод ко-ва ошибок
        if wrong > 0:
...
def main():
    ...
    # название
    pygame.display.set_caption('Sudoku v1')
    # шрифты
    font = pygame.font.SysFont('Bahnschrift', 40)
                ...
                if board.board[selected[1]][selected[0]] == 0 and selected != (-1, -1):
                    ...
                    # подтвердить выбр цифры
                        ...
                        if selected in keyDict:
                            ...
                            del keyDict[selected]
                # показать любую правильную цифру
